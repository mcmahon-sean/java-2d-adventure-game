...
public class UI {
	...
	public void draw(Graphics2D g2) {
		...
		g2.setFont(purisaB);
		g2.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON); // Removes
																											// the
																											// jagginess.
	...
	public void drawMessage() {
		...
		for (int i = 0; i < message.size(); i++) {
			if (message.get(i) != null) {
				...
				int counter = messageCounter.get(i) + 1; // array messageCounter++
	...
	public void drawInventoryScreen() {
		...
		for (int i = 0; i < gp.player.inventory.size(); i++) {
			...
			if (i == 4 || i == 9 || i == 14) { // Change to the next row
				slotX = slotXstart;
				slotY += slotSize;
			}
		}
		...
		if (itemIndex < gp.player.inventory.size()) {
			...
			drawSubWindow(dFrameX, dFrameY, dFrameWidth, dFrameHeight); // Only create item sub-window when item is selected

			for (String line: gp.player.inventory.get(itemIndex).description.split("\n")) { // Splits the line in item description
